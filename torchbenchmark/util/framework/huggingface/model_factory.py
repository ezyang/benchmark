...
class HuggingFaceGenerationModel(HuggingFaceModel):
    ...
    def train(self):
        raise NotImplementedError("generation is inference only concept")

    def eval(self):
        assert False
        out = self.generate(self.example_inputs['input_ids'], generation_config=self.generation_config)
        return (out,)
